...
- **`arrow_direction`:** Direction from the sign post that this rule applies to. **CRITICAL:** Only use the arrow that appears on the same plate as this rule. Do NOT borrow arrows from plates of a different category. If the plate has no arrow symbol and no same-category sibling has one, set `"both"` — a sign with no arrow means the restriction applies to the entire street (both directions from the post). Similarly, if a plate shows two arrows pointing in opposite directions, set `"both"`. Otherwise inherit the arrow from the same-category sibling.
